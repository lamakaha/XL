...
# Function to run operations in the same thread as UI with progress updates
def run_with_ui_updates(func, status_label, btn_text, root):
    ...
    try:
        # Run the function directly (no threading)
        result = func()

        # Get elapsed time for final status
        ...
        # Re-raise the exception to be caught by the button handler
        raise e
...
def func7():
    """Dummy function for reporting that processes files sequentially."""
    # Create a simple message in Excel
    wb = xw.Book.caller()
    ...
    sheet = wb.sheets[sheet_name]
    sheet.clear()
    sheet.range("A1").value = "Sequential Reporting Function Called"
    sheet.range("A2").value = "Date: " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    # Simulate reading multiple files sequentially
        ...
        def button_handler():
            ...
            try:
                ...
                try:
                    ...
                    # Run the function with UI updates (single-threaded)
                    run_with_ui_updates(func, status_label, btn_text, root)
                except Exception as e:
...
if __name__ == "__main__":
    ...
    # python myproject_single_thread.py --tabs=Market~Portfolio
